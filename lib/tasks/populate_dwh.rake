namespace :db do
    ...
    task :fact_quote do
        ActiveRecord::Base.establish_connection(:adapter  => "mysql2",:host => "localhost", :username => ENV["MYSQL_USERNAME"],:password => ENV["MYSQL_PASSWORD"],:database => "Rocket_Elevators_Information_System_development")
        row = ActiveRecord::Base.connection.execute('SELECT q.id, q.created_at ,q.compagnyName ,q.email ,q.numElevator 
                                                    FROM quotes q; ')
        ActiveRecord::Base.establish_connection(:adapter  => "postgresql",:host => "localhost", :username => ENV["POSTGRESQL_USERNAME"],:password => ENV["POSTGRESQL_PASSWORD"],:database => "data_warehouse")
        row.each do |val|
